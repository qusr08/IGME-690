...
namespace ProceduralMesh.Generators
{
    public struct Octasphere : IMeshGenerator
    {
        ...
        public void ExecuteRegular<S>(int i, S streams) where S : struct, IMeshStream
        {
            int u = i / 4;
        ...
        public void ExecutePolesAndSeam<S>(S streams) where S : struct, IMeshStream
        {
            Vertex vertex = new Vertex();
        ...
        private static float2 GetTangentXZ(float3 p)
        {
            return normalize(float2(-p.z, p.x));
        }

        private static float2 GetTexCoord(float3 p)
        {
            float2 texCoord = float2(atan2(p.x, p.z) / (-2f * PI) + 0.5f, asin(p.y) / PI + 0.5f);
            if (texCoord.x < 1e-6f)
            {
                texCoord.x = 1f;
            }
